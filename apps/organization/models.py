...
class CityDict(models.Model):
    ...
    def __unicode__(self):
        return  self.name   #作用：在xadmin后台显示城市名而不是CityDict
...
class CourseOrg(models.Model):
    ...
    city =models.ForeignKey(CityDict,verbose_name=u's所在城市')
    add_time = models.DateTimeField(default=datetime.now)
    ...
    def __unicode__(self):
        return self.name  # 作用：在xadmin后台显示城市名而不是
...
class Teacher(models.Model):
    ...
    add_time =models.DateTimeField(default =datetime.now)
    image = models.ImageField(upload_to='teacher/%Y/%m',verbose_name='教师形象照片',max_length=100, blank=True)
